...
class HtmlSheetWriter(BaseSheetWriter):
    ...
    def _write(self, k, directory, filename, logger):
        ...
        print("<html>", file=f)
        print("<!--\n    Generated by GNU Solfege %s\n-->" % buildinfo.VERSION_STRING, file=f)
        print("<link rel='stylesheet' href='style.css' type='text/css'/>", file=f)
    ...
    def write_to(self, directory, logger):
        ...
        print("table { border: none }", file=f)
        print(".answer { text-align: center }", file=f)
        f.close()
    ...
    def _write(self, k, directory, filename, logger):
        ...
        for section in self.m_data:
            ...
            for idx, question_dict in enumerate(section):
                if idx % section.m_line_len == 0:
                    if first_row:
                        print(r"\begin{tabular}{%s}" % ('c' * section.m_line_len), file=f)
                        first_row = False
                    else:
        ...
        f = open(os.path.join(directory, 'README.txt'), 'w')
        print("\n".join(textwrap.wrap("The files you are looking for are lilyout%(sep)sanswers.dvi and lilyout%(sep)squestions.dvi" % {'sep': os.path.sep})), file=f)
        f.close()
